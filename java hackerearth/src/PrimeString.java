    ...
    public static void main(String[] args) {
      ...
      */
        ...
        while (noOfTestCases-->0)
        {
            ...
            HashMap<Character,Integer> occurenenceCount=new HashMap<>();
            for(int i=0;i<s.length();i++) {
                if (!occurenenceCount.containsKey(s.charAt(i))) {
                    occurenenceCount.put(s.charAt(i), 1);
                } else {
                    occurenenceCount.replace(s.charAt(i),occurenenceCount.get(s.charAt(i))+1);
                }
            }
                boolean isValid=true;
                if(isPrime[occurenenceCount.size()])
                {
                    for(int count:occurenenceCount.values())
                    {
                        if(!isPrime[count])
                        {
                            isValid=false;
