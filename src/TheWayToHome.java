...
public class TheWayToHome {
    public static void main(String[] args) {
      ...
      */
        ...
        while (true)
        {
            currPoint=nextGreatestValidPoint(path,currPoint,d);
            if(currPoint==-1)
            {
                out.print("-1");
                out.close();
   ...
   static int nextGreatestValidPoint(String path,int start,int d)
    {
        int nextGreatest=-1;
        for(int i=start+1;(i-start)<=d && i<path.length();i++)
        {
            if(path.charAt(i)=='1')
                nextGreatest=i;
        }
